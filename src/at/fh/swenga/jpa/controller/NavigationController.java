...
public class NavigationController {
	...
	public String users(Model model, Principal principal) {
		...
		// List with all users that are not disabled, except guest user, because guest
		// user is no real user
		List<User> users = userDao.getAllUsers();
		// List with every user except guest user, because if admin accidently disables
		// guest user, log in as guest won't work
		if (!isAdmin)
			users = userDao.getAllEnabledUsers();
