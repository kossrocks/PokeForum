...
public class CalculatorController {
	...
	public String calculate(Model model, Principal principal, @RequestParam("yourPokemon") int yourPokemonId,
			@RequestParam("yourLevel") float yourLevel, @RequestParam("yourAttack") int attackId,
			@RequestParam("enemyPokemon") int enemyId, @RequestParam("enemyLevel") float enemyLevel) {
		...
		// set the level of pokemon in team. It will not be changed in the database.
		// This change only is for calculating purpose
		yourPokemon.setLevel(yourLevel);
		// due to the changed level, its stats need to be recalculated
		yourPokemon.recalculateStats();

		// depending on the attack category slightly different function variables are
		...
		// if a pokemon has the same type as the attack it is using, it gets an attack
		// bonus
		...
		damage = ((yourLevel * (2f / 5f) + 2f) * yourAttack.getBasePower() * (yourAtkpoints / (50f * enemyDefpoints))
				+ 2f) * stab * adv;

		// reamaining enemy healthpoints
		float enemyHp = (2f * enemyPokemon.getBaseHealthPoints() * enemyLevel / 100f) + 10f + enemyLevel - damage;

		// for more user friendly display of remaining hp of enemy. User associates
		// enemy defeat with 0 hp.
		if (enemyHp < 0)
			enemyHp = 0;

		model.addAttribute("message", "Your attack made " + Math.round(damage) + " damage and your enemy has "
				+ Math.round(enemyHp) + " HP left.");
