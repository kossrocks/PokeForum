...
public class EntryDao {
	...
	public List<EntryModel> getAllEntries() {
		TypedQuery<EntryModel> typedQuery = entityManager.createQuery("select e from EntryModel e", EntryModel.class);
		List<EntryModel> typedResultList = typedQuery.getResultList();
		return typedResultList;
	}

	public List<EntryModel> getAllEntriesInTopic(int id) {
		TypedQuery<EntryModel> typedQuery = entityManager
				.createQuery("select e from EntryModel e where e.topic.id = :id order by e.id", EntryModel.class);
	...
	public int delete(int id) {
		int count = entityManager.createQuery("DELETE FROM EntryModel WHERE id = :id").setParameter("id", id)
				.executeUpdate();
		return count;
	}

	public int deleteById(int id) {
		int count = entityManager.createQuery("DELETE FROM EntryModel WHERE topic.id = :id").setParameter("id", id)
				.executeUpdate();
		return count;
	}
}
