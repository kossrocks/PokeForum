...
public class UserDao {
	...
	public List<User> getAllUsers() {
		TypedQuery<User> typedQuery = entityManager
				.createQuery("select u from User u where not u.userName = 'guest' order by u.id", User.class);
		List<User> typedResultList = typedQuery.getResultList();
		return typedResultList;
	}
	...
	public int addIt(User user) {
		int count = entityManager.createQuery(
				"INSERT INTO UserModel (userName,password,enabled,dateOfEntry) VALUES (:username,:password,:enabled,:dateOfEntry)")
				.setParameter("username", user.getUserName()).setParameter("password", user.getPassword())
				.setParameter("enabled", user.isEnabled()).setParameter("dateOfEntry", user.getDateOfEntry())
				.executeUpdate();
		return count;
	}
	...
	public List<User> getAllEnabledUsers() {
		TypedQuery<User> typedQuery = entityManager.createQuery(
				"select u from User u where not u.userName = 'guest' and not u.enabled = 0 order by u.id", User.class);
		List<User> typedResultList = typedQuery.getResultList();
		return typedResultList;
	}
